...
| module_data | JSON | 模块内的表单数据 | - |
| updated_at | TIMESTAMP | 最后更新时间 | - |

> **设计备注**：使用 JSON 是为了适应 AI 项目孵化不同阶段差异巨大的数据结构（如创意阶段存文本，UI阶段存配色对象），避免频繁修改表结构。
...
- **策略**：使用 **JSON** 存储 `project_modules.module_data`（当前后端落地为 MySQL）。
- **优势**：
...
- **索引策略**：
    ...
    - 唯一索引：防止重复点赞、重复加入团队。
- **全文检索**：对于 `project_modules` 的 JSON 内容和 `project_files` 的长文本，建议结合 ElasticSearch；如坚持 MySQL，可评估 FULLTEXT 或“生成列(JSON_EXTRACT)+索引”的折中方案。
